...
# Mapping Endpoint ke 8 Tabel Modular
ENDPOINT_TABLE_MAP = {
...
def extract_accurate_incremental(dynamic_host, endpoint, helper, start_date_str, end_date_str):
    ...
    while True:
        ...
        # Parameter dinamis untuk Incremental.
        # Catatan: Beberapa endpoint Accurate menggunakan filter.modifiedAt.gte atau filter.transDate.gte
        # Anda mungkin perlu menyesuaikan nama parameter filter ini sesuai respon OpenAPI Accurate.
        params = {
            "sp.page": page,
...
def load_to_accurate_table(table_name, endpoint, raw_data):
    ...
    logger.info(f"[{endpoint}] berhasil dimuat ke tabel {table_name}. Total: {len(raw_data)} baris.")

@flow(name="Accurate_Incremental_ETL_Flow")
def accurate_incremental_etl():
    logger = get_run_logger()
    ...
    # Menentukan Rolling Window (H-3 sampai Hari Ini)
    # Accurate menggunakan format dd/mm/yyyy
    ...
    for endpoint, target_table in ENDPOINT_TABLE_MAP.items():
