...
from src.transform._audit import pre_audit_fact_sales_online
...
def run(engine, marketplace):
    ...
    try:
        with engine.begin() as conn:
            ...
            pre_audit_fact_sales_online(conn, marketplace)
            ...
            if marketplace == 'shopee':
                result = conn.execute(text("""
                    ...
                    DO UPDATE SET
                        ...
                        amt_product_discount = COALESCE(EXCLUDED.amt_product_discount, fact_sales_online.amt_product_discount),
                        amt_gross_revenue    = COALESCE(EXCLUDED.amt_gross_revenue,    fact_sales_online.amt_gross_revenue),
                ...
                result = conn.execute(text("""
                    INSERT INTO public.fact_sales_online (
                        ...
                        SELECT DISTINCT ON (o.order_number, dp.product_id, cm.sales_channel_id)
                            ...
                            FALSE,
                            1,
                            CASE WHEN TRIM(o.status) IN ('returned','Package Returned',
                                'In Transit: Returning to seller') THEN 1 ELSE 0 END,
