        ...
        with engine.begin() as conn:
            ...
            if marketplace == 'shopee':
                result = conn.execute(text("""
                    ...
                    DO UPDATE SET
                        amt_settled         = COALESCE(EXCLUDED.amt_settled, fact_settlement.amt_settled),
                        time_funds_released = COALESCE(EXCLUDED.time_funds_released, fact_settlement.time_funds_released),
                ...
                result = conn.execute(text("""
                    INSERT INTO public.fact_settlement (
                        ...
                        SELECT DISTINCT ON (o.order_adjustment_id, cm.sales_channel_id)
                        ...
                        WHERE cm.sales_channel_id IS NOT NULL
                            ...
                            AND o.type = 'Order'
                        ORDER BY o.order_adjustment_id, cm.sales_channel_id, o.uploaded_at DESC NULLS LAST
                    ON CONFLICT (order_id, sales_channel_id)
                    DO UPDATE SET
                        amt_settled         = COALESCE(EXCLUDED.amt_settled, fact_settlement.amt_settled),
                        time_funds_released = COALESCE(EXCLUDED.time_funds_released, fact_settlement.time_funds_released),
                ...
                result = conn.execute(text("""
                    INSERT INTO public.fact_settlement (
                        ...
                        SELECT DISTINCT ON (sub.order_id, sub.sales_channel_id)
                            sub.order_id,
                        ...
                        FROM (
                            ...
                            GROUP BY 1, 2, o.source_filename, o.uploaded_at
                        ) sub
                        ORDER BY sub.order_id, sub.sales_channel_id, sub.uploaded_at DESC NULLS LAST
                    ON CONFLICT (order_id, sales_channel_id)
                    DO UPDATE SET
                        amt_settled         = COALESCE(EXCLUDED.amt_settled, fact_settlement.amt_settled),
                        time_funds_released = COALESCE(EXCLUDED.time_funds_released, fact_settlement.time_funds_released),
